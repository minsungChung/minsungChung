...
sudo docker pull juhwan0815/wjm-server:latest

echo "> 도커 실행 포트:$IDLE_PORT"
sudo docker run -d --name $IDLE_PORT -p ${IDLE_PORT}:${IDLE_PORT} -e "server.port=${IDLE_PORT}" juhwan0815/wjm-server:latest
